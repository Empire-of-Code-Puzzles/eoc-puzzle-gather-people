"""
TESTS is a dict with all you tests.
Keys for this will be categories' names.
Each test is dict with
    "input" -- input data for user function
    "answer" -- your right answer
    "explanation" -- not necessary key, it's using for additional info in animation.
"""

TESTS = {
    "Basics": [
        {
            "input": [[[0, 40, 0, 40, 0, 0], [40, 6, 0, 0, 40, 0], [0, 0, 3, 0, 28, 0], [40, 0, 0, 4, 40, 28],
                       [0, 40, 28, 40, 1, 0], [0, 0, 0, 28, 0, 2]], 80],
            "answer": 13,
            "rooms": [[1, 1, 0], [5, 1, 6], [7, 3, 3], [1, 5, 4], [5, 5, 1], [3, 7, 2]],
            "paths": [[0, 1], [0, 3], [1, 4], [2, 4], [3, 4], [3, 5]],
            "escaped": [0, 3, 5, 4, 1],
        },
        {
            "input": [[[0, 40, 0, 40, 0, 0], [40, 6, 0, 0, 40, 0], [0, 0, 3, 0, 28, 0], [40, 0, 0, 4, 40, 28],
                       [0, 40, 28, 40, 1, 0], [0, 0, 0, 28, 0, 2]], 40],
            "answer": 10,
            "rooms": [[1, 1, 0], [5, 1, 6], [7, 3, 3], [1, 5, 4], [5, 5, 1], [3, 7, 2]],
            "paths": [[0, 1], [0, 3], [1, 4], [2, 4], [3, 4], [3, 5]],
            "escaped": [0, 3, 1],
        },
        {
            "input": [[[0, 40, 0, 40, 0, 0], [40, 6, 0, 0, 40, 0], [0, 0, 3, 0, 28, 0], [40, 0, 0, 4, 40, 28],
                       [0, 40, 28, 40, 1, 0], [0, 0, 0, 28, 0, 2]], 39],
            "answer": 0,
            "rooms": [[1, 1, 0], [5, 1, 6], [7, 3, 3], [1, 5, 4], [5, 5, 1], [3, 7, 2]],
            "paths": [[0, 1], [0, 3], [1, 4], [2, 4], [3, 4], [3, 5]],
            "escaped": [0],
        },
        {
            "input": [[[1, 0, 40, 0, 40, 40, 0, 40, 0], [0, 1, 40, 0, 40, 0, 0, 0, 0], [40, 40, 1, 40, 0, 0, 0, 0, 0],
                       [0, 0, 40, 1, 0, 89, 0, 0, 0], [40, 40, 0, 0, 1, 0, 40, 0, 0], [40, 0, 0, 89, 0, 1, 0, 0, 40],
                       [0, 0, 0, 0, 40, 0, 1, 40, 0], [40, 0, 0, 0, 0, 0, 40, 1, 40], [0, 0, 0, 0, 0, 40, 0, 40, 1]],
                      40],
            "answer": 5,
            "rooms": [[5, 5, 1], [1, 1, 1], [5, 1, 1], [9, 1, 1], [5, 1, 1], [5, 9, 1], [9, 1, 1], [9, 5, 1],
                      [9, 9, 1]],
            "paths": [[1, 2], [2, 3], [4, 0], [0, 5], [6, 7], [7, 8], [1, 4], [2, 0], [3, 5], [4, 6], [0, 7], [5, 8]],
            "escaped": [0, 7, 5, 4, 2],
        },
        {
            "input": [[[1, 0, 40, 0, 40, 40, 0, 40, 0], [0, 1, 40, 0, 40, 0, 0, 0, 0], [40, 40, 1, 40, 0, 0, 0, 0, 0],
                       [0, 0, 40, 1, 0, 89, 0, 0, 0], [40, 40, 0, 0, 1, 0, 40, 0, 0], [40, 0, 0, 89, 0, 1, 0, 0, 40],
                       [0, 0, 0, 0, 40, 0, 1, 40, 0], [40, 0, 0, 0, 0, 0, 40, 1, 40], [0, 0, 0, 0, 0, 40, 0, 40, 1]],
                      1000],
            "answer": 9,
            "rooms": [[5, 5, 1], [1, 1, 1], [5, 1, 1], [9, 1, 1], [5, 1, 1], [5, 9, 1], [9, 1, 1], [9, 5, 1],
                      [9, 9, 1]],
            "paths": [[1, 2], [2, 3], [4, 0], [0, 5], [6, 7], [7, 8], [1, 4], [2, 0], [3, 5], [4, 6], [0, 7], [5, 8]],
            "escaped": [0, 7, 8, 5, 3, 2, 1, 4, 6],
        },
    ],
    "Extra": [
        {
            "input": [6, 3],
            "answer": 9,
            "explanation": "6+3=?"
        },
        {
            "input": [6, 7],
            "answer": 13,
            "explanation": "6+7=?"
        }
    ]
}
